...
export function PageLayout({
  ...
  // determine if we're on mobile size
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 transform transition-transform duration-30 ease-in-out lg:hidden ${isMobileDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
          ...
          <div className="flex h-16 items-center px-4">
            ...
            {tabs && tabs.length > 0 && (
              <div className="ml-4 flex items-center">
                {tabs.map((tab) => (
                  <Link
                    className={`full mr-2 rounded px-3 py-1 text-sm transition-colors duration-200 ${pathname === tab.path ? "bg-black text-white" : "text-gray-500 hover:text-gray-700"}`}
                    key={tab.path}
                    href={tab.path}
                  >
                    {tab.name}
