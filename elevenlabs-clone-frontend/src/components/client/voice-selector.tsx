...
export function VoiceSelector({ service }: { service: ServiceType }) {
  ...
  // these retrieve the function from the voice store
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        ...
        className="hover:bg-opacity flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2 hover:cursor-pointer hover:bg-gray-100"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 left-0 z-10 mt-1 max-h-60 overflow-auto rounded-lg border border-gray-200 shadow-lg">
          {voices.map((voice) => (
            <div
              key={voice.id}
